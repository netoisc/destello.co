...
-- Storage bucket for audio files (run this separately in Storage section or SQL Editor)
-- Note: The 'audio' bucket should be created through the Supabase UI (Storage section)
-- Make sure to:
-- 1. Create bucket named 'audio'
-- 2. Make it PUBLIC
-- 3. Set appropriate file size limits
-- 4. Add policies for public read/write access

-- Storage policies (run these if needed after creating the bucket)
