...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, ...props }, ref) => {
    return (
      <textarea
        className={cn(
          "flex min-h-[140px] w-full rounded-xl border border-white/10 bg-black/40 backdrop-blur-md px-4 py-3 text-sm text-white placeholder:text-gray-500 focus-visible:outline-none focus-visible:border-purple-400/50 focus-visible:bg-black/60 focus-visible:shadow-[0_0_20px_rgba(168,85,247,0.3)] transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-50 resize-none relative overflow-hidden",
